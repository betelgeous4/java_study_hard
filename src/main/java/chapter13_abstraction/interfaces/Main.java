...
/*
    인터페이스(interface)
        인터페이스는 자바에서 클래스가 구현해야 하는 '메서드들'의 집합을 정의하는 일종의 규약(protocol)이다.
        인터페이스는 메서드의 시그니처( 메서드 이름, 리턴타입, 매개변수 목록)만을 포함하며, 메서드의 실제 구현은
        포함하지 않는다. 이 점이 추상 메서드와 유사한 점이다

        다중 상속을 지원 -> 이를 통해 클래스가 여러 인터페이스를 구현할 수 있다 : 추상 클래스는 단일 상속을
        지원해 상속 과정을 알아보기 쉽다 -> 차이점

        특징
            1. 추상 메서드 : 인터페이스 내의 모든 메서드는 기본적으로 추상 메서드 (인터페이스는 전부 다 추상메서드이다)
            2. 상수 : 인터페이스 내에서 선언된 변수는 모두 '자동으로'
            public static final 상수로 취급됨
            3. 다중 상속 : 클래스는 여러 인터페이스를 구현할 수 있다.

        추상 클래스와의 공통점
            - 모두 추상 클래스를 포함할 수 있고, 이를 서브 클래스에서 구현애햐 한다.
        ...
        추상 클래스와의 차이점
            ...
            - 인터페이스
                1. 완전한 추상화 : 인터페이스는 기본적으로 모든 메서드가 추상메서드로, Java8 이후로 default, static 메서드를 사용할 수 있다.
                2. 인터페이스는 인스턴스 변수(필드 중 객체마다 값이 다른 변수)를 가질 수 없고, 상수만 선언 가능
                3. 다중 상속 : 클래스는 여러 인터페이스를 '구현할 수 있다'
                4. 생성자 없음 : 인터페이스는 생성자를 가질 수 없다.
                    -> 일단 필드에서 객체마다 다른 값을 가지게 되는 인스턴스 변수가 없다 -> 생성자 생성이 불가능 ->
                    객체마다 다른 값을 가질 수 없다.
                5. 자동 public : 인터페이스의 메서드들은 자동으로 public 이며, 메서드 선언에 접근 제어자를 명시할 필요가 없다,.
                Java 8 이후에 default, static 메서드 사용할 수 있다.
 */
public class Main {
    public static void main(String[] args) {
        ...
        /*
            ...
            매개변수 생성자로 정의했기 때문에 오류가 발생한다. 그래서 argument 들을 채워넣을 필요가 있는데,
            RemoteController 의 객체를 생성했고, 객체명은 remoteController 이다.
            클래스명1 객체명1 = new 클래스명1(argument1, argument2, ... );
            클래스명2 객체명2 = new 클래스명1(new 클래스명3() , new 클래스명4() , ... );

            이상의 경우 new 클래스명3(), new 클래스명 4()를 통해 객체 생성은 이미 '완료'되었다
        ...
        remoteController.onPressedPowerButton();
        /*
            다중 상속을 지원하지 않는 Java 클래스에서 remoteController 라는 객체가
            PowerButton / VD / VU 에 있는 메서드 5개를 전부 호출할 수 있도록 하기 위해
            필드로 Pb / VU / VD 을 가지고, 생성자의 argument 로 사용한 다음,
            RemoteController 의 일반 메서드로 PB의 메서드 호출을 할 수 있도록 정의

            그리고 Main.java 에서 remoteController.onPressedPowerButton(); 로 호출하면,
            onPressedPowerButton(); 이 powerButton.onPressed(): 를 따로 호출해,
            이중 호출을 하는 방식으로 전체 기능을 구현할 수 있도록 작성
         */
